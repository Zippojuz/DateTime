...
| Corps differentiation pass | — | ✅ DONE (1984 pass, per user). Each corp advertises in its own voice — 5 bespoke ads each in corps.json (Oceania: telescreens that love you, Victory Chai, "NOTHING TO HIDE, NOTHING TO FEAR, NOTHING ELSE"; Eurasia: quota parades, "YOUR BODY IS 60% REPLACEABLE"; Eastasia: ads that whisper into your lace, "TWO PLUS TWO IS WHATEVER YOU NEED IT TO BE"), plus a per-corp Ministry Holdings *denial*. The weekly war is now visible: war_state grew a joint-statement `bulletin` ("Records indicating otherwise have been corrected"), the Lookout carries The War row, and the **Triumvirate Exchange** venue renders three color-coded storefront cards with ALLIED/THE ENEMY badges (ExchangeView). Plant 7 is a **researchable buried file at the Stacks** (event found:plant_7, once per save): the freight manifest, Ministry Holdings, THE WAR IS THE PRODUCT — the conspiracy thread for future arcs. Records Clerk became Records *Correction* Clerk (memory-hole day job). |
| The Forget-Me-Not (pawnshop venue) | — | ✅ DONE: the Shallows, 06:00–00:00 (game/pawnshop.py, migration 18: player.pawned). SELLING exists: broker pays 45% of sticker (value × rarity); pawned items wait on the shelf (cap 12), buyback at ×1.25 for 7 days, then sold on. Refusal lines for the cyberlink (EULA), the lens (a habit), the bell (some things the shop won't hold). Chassis trait line (the window tray). The locked back case — MEMORIES, BY APPOINTMENT ONLY — is the standing hook for the memory-economy stretch; no mechanics yet, on purpose. |
